...
export default function TenantContractsPage() {
  const [contracts, setContracts] = useState(CONTRACTS_DB);
  ...
  // giả lập data tenant (sau này sẽ fetch API /users?role=TENANT)
  const [tenants] = useState(TENANTS_DB);
  ...
  // search theo mã HĐ, phòng, tên tenant, sđt tenant
  ...
  function handleDelete(id) {
    ...
    const ok = window.confirm(
      `Bạn có chắc chắn muốn xóa hợp đồng ${target.code}?`
    );
    if (!ok) return;
              ...
              <tbody>
                {filteredContracts.length === 0 ? (
                  ...
                  filteredContracts.map((contract) => {
                    ...
                    if (contract.status === "expired" || end < today) {
                      statusLabel = "Hết hạn";
                      statusClass =
                        "bg-amber-50 text-amber-700 border-amber-100";
                    } else if (diffDay <= 30 && diffDay >= 0) {
